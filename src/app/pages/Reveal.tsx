                ...
                <div className={isTie ? "grid gap-3 sm:grid-cols-2" : ""}>
                  {winners.map((winner, index) => (
                    <motion.div
                      ...
                      className="relative overflow-hidden rounded-[20px] border-2 border-primary/40 bg-card p-8"
                      style={{ boxShadow: "0 0 40px rgba(92, 107, 255, 0.3)" }}
                    >
                      <div className="absolute right-4 top-4">
                        <Star className="h-8 w-8 fill-yellow-400 text-yellow-400" />
                      </div>
                      <p className="mb-2 text-center text-sm font-bold uppercase text-primary">{isTie ? "Tie winner" : "Winner"}</p>
                      <h1 className="mb-2 text-center text-3xl font-bold">{winner.option.label}</h1>
                      <p className="text-center text-muted-foreground">{winner.yes} yes votes</p>
                    </motion.div>
                  ))}
                </div>
